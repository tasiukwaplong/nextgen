...
xl = pd.ExcelFile('3MTT NextGen Project Brief Bank.xlsx')

# 1. Master Index
master_df = xl.parse('Master Index', header=None)
master_dict = {}
header_idx = -1
for r in range(len(master_df)):
    val = str(master_df.iloc[r, 0]).strip().lower()
    if 'brief id' in val:
        header_idx = r
        break

if header_idx != -1:
    for r in range(header_idx + 1, len(master_df)):
        row = master_df.iloc[r]
        b_id = str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else ''
        if b_id and b_id.lower() != 'nan':
            track = str(row.iloc[1]).strip() if len(row) > 1 and pd.notna(row.iloc[1]) else ''
            title = str(row.iloc[2]).strip() if len(row) > 2 and pd.notna(row.iloc[2]) else ''
            diff = str(row.iloc[3]).strip() if len(row) > 3 and pd.notna(row.iloc[3]) else ''
            master_dict[b_id] = {'track': track, 'title': title, 'difficulty': diff}

track_sheets = [
    'AI - Machine Learning', 'Animation', 'Cloud Computing', 'UI-UX Design',
    'Data Analysis and Visualization', 'Data Science', 'DevOps', 'Game Development',
    'Product Management', 'Quality Assurance', 'Software Development', 'Cybersecurity'
]

all_briefs = []

for sheet in track_sheets:
    df = xl.parse(sheet, header=None)
    header_idx = -1
    for r in range(len(df)):
        val = str(df.iloc[r, 0]).strip().lower()
        if 'brief id' in val:
            header_idx = r
            break
    if header_idx != -1:
        ...
        for r in range(header_idx + 1, len(df)):
            ...
            if b_id and b_id.lower() != 'nan':
                brief = {
                    ...
                    'deliverables': '',
                    'tools': '',
                    'difficulty': master_dict.get(b_id, {}).get('difficulty', 'Beginner')
                }
                for c_idx, col_name in enumerate(cols):
                    if pd.notna(col_name) and col_name and c_idx < len(row):
                        val = str(row.iloc[c_idx]).strip() if pd.notna(row.iloc[c_idx]) else ''
                        col_lower = col_name.lower()
                        if 'title' in col_lower: brief['title'] = val
                        elif 'problem' in col_lower: brief['problem'] = val
                        elif 'what to build' in col_lower: brief['solution'] = val
                        elif 'core' in col_lower or 'feature' in col_lower: brief['features'] = val
                        elif 'deliverable' in col_lower: brief['deliverables'] = val
                        elif 'tool' in col_lower: brief['tools'] = val
                        elif 'difficulty' in col_lower and val: brief['difficulty'] = val
                
                if not brief['title'] and b_id in master_dict:
                    brief['title'] = master_dict[b_id]['title']
                all_briefs.append(brief)

# 2. Data Sources
ds_df = xl.parse('Data Source Index', header=None)
data_sources = []
header_idx = -1
for r in range(len(ds_df)):
    val = str(ds_df.iloc[r, 0]).strip().lower()
    if 'source' in val:
        header_idx = r
        break

if header_idx != -1:
    for r in range(header_idx + 1, len(ds_df)):
        row = ds_df.iloc[r]
        s_name = str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else ''
        if s_name and s_name.lower() != 'nan':
            desc = str(row.iloc[1]).strip() if len(row) > 1 and pd.notna(row.iloc[1]) else ''
            url = str(row.iloc[2]).strip() if len(row) > 2 and pd.notna(row.iloc[2]) else ''
            data_sources.append({'name': s_name, 'description': desc, 'url': url})

briefs_json_str = json.dumps(all_briefs, ensure_ascii=False)
sources_json_str = json.dumps(data_sources, ensure_ascii=False)
...
<head>
    ...
    <meta name="description" content="Interactive Project Brief Bank, Search, Category Filter, Sort and Evaluation Rubric for 3MTT NextGen Cohort.">
    <!-- Google Fonts -->
...
print("Updated index.html and 3MTT_NextGen_Project_Brief_Bank.html successfully!")
